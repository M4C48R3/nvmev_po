...
if __name__ == '__main__':
	...
	checkpoint_file = f"./output/checkpoints/checkpoint {int(time.time())}.pkl"
	checkpoint_saver = skopt.callbacks.CheckpointSaver(checkpoint_file)
	LOAD = "./output/checkpoints/checkpoint 1691380588.pkl" # put file to load (./output/checkpoints/checkpoint 1691380588.pkl), if not loading a previous result from a file, set to 0
	if LOAD:
		res = skopt.load(LOAD)
		res = skopt.optimizer.gp_minimize(
			func=get_params.get_params, dimensions=skopt_dim,
			initial_point_generator="hammersly", n_calls=45, n_random_starts=6,
			verbose=True, callback=[checkpoint_saver],
			x0=res.x_iters, y0=res.func_vals
		)
	else:
		res = skopt.optimizer.gp_minimize(
			func=get_params.get_params, dimensions=skopt_dim,
